...
function convertToJSON(text) {
  ...
  const data = lines.slice(1).map(line => {
    const values = line.match(/(.+?)\s{2,}(Male|Female)\s{2,}(.+?)\s{2,}(.+)$/);
    if (!values) return null;

    return {
      [headers[0]]: values[1].trim(),
      [headers[1]]: values[2].trim(),
      [headers[2]]: values[3].trim(),
      [headers[3]]: values[4].trim()
    };
...
const fs = require('fs');
fs.writeFileSync('voiceList.json', jsonString);
