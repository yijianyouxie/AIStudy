...
## 2025-11-11；确定下Unity20174.31f1下不能流式播放mp3格式的音频吗？如果用mp3或wav各有什么优势和缺点？
## 2025-11-12；原来Unity 2017.4.31f1下可以流式播放mp3文件，且能边下边播。（遇到问题一定要认真求证）

# 开发说明 2025-11-13
## 这个工程运行的命令是pnpm build pnpm start这两个命令，这是按照生产版本需要运行的命令导致后续【frac库导入问题是通过升级项目】
## 这样的问题以及解决方式。
## 实际上在本地开发运行命令：pnpm dev就可以了。所以EasyVoice2工程应运而生。
